...
Skills 是**平台无关**的 —— 同一份 `SKILL.md` 可以在 Codex、Claude Code、Cursor 等支持 Markdown 指令的 AI 工具中使用。首发自动安装器固定支持这三个客户端。

## 目录结构

发布给用户的 Skills 位于 `advjs` npm 包的完整性目录中；仓库根目录的 `skills/` 是它们的源码：
...
| [`adv-hamster-demo`](https://github.com/YunYouJun/advjs/blob/main/skills/adv-hamster-demo/SKILL.md) | —      | 仅限 `demo/hamster` 的无缝主线、回声演算、COS 路径与发布验收约束 |
