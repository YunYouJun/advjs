...
::: tip 场景 vs 地点
**场景**（`adv/scenes/`）是叙事中的视觉单元，包含背景图/3D 模型和 AI 图片生成提示词，通过 `【场景名，时间，内/外景】` 语法在游戏脚本中引用。

**地点**（`adv/locations/`）是世界观中的地理实体，包含关联关系和地理分类，用于 Studio 中组织世界观并追踪 AI 聊天中的角色位置。

一个地点可以对应多个场景（同一咖啡厅的"早晨"和"夜晚"是两个不同场景），两者互补而非替代。
:::
...
- **Tools**：`adv_validate` + 7 个新增 CRUD 工具（共 8 个）
- **Prompts**：`write-chapter`、`create-character`、`review-script`（3 个创作辅助提示）

新增工具：

- [x] **`list_files`** — 列出项目文件结构（角色/章节/场景/地点/知识文件清单）
- [x] **`create_character`** — 创建新角色卡片（写入 `.character.md` 文件）
- [x] **`edit_character`** — 编辑已有角色卡片（frontmatter + body 更新）
- [x] **`create_chapter`** — 创建新章节脚本（写入 `.adv.md` 文件）
- [x] **`edit_chapter`** — 编辑已有章节脚本
- [x] **`project_stats`** — 项目统计信息（角色数、章节数、场景数、字数统计）
- [x] **`search_content`** — 全文搜索项目内容（按分类过滤）

**技术实现**：

- `packages/mcp-server/src/index.ts` — 复用 `@advjs/parser` 的 `parseCharacterMd`/`stringifyCharacterMd` 进行角色文件读写，复用 `advjs` 的 `scanFiles` 遍历目录
- 所有写入工具均有文件存在性检查（create 不覆盖，edit 需文件存在），`sanitizeFilename` 防止路径穿越
- `search_content` 支持按 category 过滤 + 大小写不敏感匹配，最多返回 50 条结果

##### 批量角色导入

- [x] **CSV 导入** — 解析 CSV 文件（name/tags/personality/background 等列），批量生成 `.character.md` 文件
- [x] **JSON 导入** — 支持 JSON 数组格式的角色数据批量导入
- [x] **导入预览** — 导入前展示解析结果预览，每行显示冲突状态
- [x] **重复检测** — 导入时检查角色 ID 冲突，提供覆盖/跳过/重命名选项

**技术实现**：

- `apps/studio/src/utils/csvParser.ts` — 零依赖 RFC 4180 CSV 解析器（支持引号字段、嵌入逗号、转义引号）
- `apps/studio/src/views/workspace/BatchImportPage.vue` — 完整导入页面（文件上传 → 格式检测 → 解析预览 → 冲突处理 → 批量写入）
- 从 CharactersPage 新增批量导入入口按钮（☁️ 图标）
- 15 个单元测试覆盖 CSV 解析边界情况
- i18n 中/英双语完整支持
...
- [x] **情感数据时序记录** — `useCharacterMemoryStore` 新增 `emotionalHistory: EmotionalSnapshot[]` 字段，每次对话后自动追加快照（最多 200 条，无需 DB 迁移）
- [x] **情感折线图组件** — `EmotionalArcChart.vue`，纯 SVG 绘制 affinity（蓝色）/ trust（绿色）趋势折线 + mood 散点，鼠标悬停显示详情
- [x] **角色详情集成** — `CharacterInfoModal` 情感状态 section 下方新增"📈 情感弧光"折叠区块
- [x] **日记情绪标注** — 时间线 diary 卡片 mood 徽章增加 emoji 前缀；`TimelineFilter` 新增 mood 过滤行（仅日记可见时显示）

**技术实现**：

- `apps/studio/src/stores/useCharacterMemoryStore.ts` — `EmotionalSnapshot` 接口 + `emotionalHistory` 字段，`extractMemoryFromTurn` 中每次更新后自动追加快照，旧数据向后兼容（无 `emotionalHistory` 时补空数组）
- `apps/studio/src/components/EmotionalArcChart.vue` — 纯 SVG 折线图（零依赖），X 轴时间 + Y 轴数值双轴，图例 + 网格线 + 鼠标提示
- `apps/studio/src/components/TimelineFilter.vue` — `TimelineFilter` 接口新增 `moods: string[]` 字段，新增 mood 过滤行 + `toggleMood` 逻辑
- `apps/studio/src/components/WorldTimeline.vue` — `availableMoods` computed + mood 过滤逻辑 + diary mood 徽章 emoji 增强

##### 导出增强

- [x] **CSV 数据导出** — 角色列表、事件时间线、关系数据导出为 CSV 表格（`csvExport.ts`）
- [x] **Markdown 增强导出** — 对话记录导出 Markdown 增强：用户消息 blockquote 格式 + 末尾附带情感状态摘要
- [ ] **EPUB 对话导出**（远期）— 对话记录打包为 EPUB 电子书格式

**技术实现**：

- `apps/studio/src/utils/csvExport.ts` — `stringifyCSV` + `exportCharactersToCSV` + `exportTimelineToCSV` + `exportRelationshipsToCSV`，RFC 4180 合规（引号转义 + 特殊字符处理）
- `apps/studio/src/views/workspace/CharactersPage.vue` — header 新增 📥 导出按钮，调用 `exportCharactersToCSV` + `downloadAsFile`
- `apps/studio/src/components/WorldEventsSection.vue` — 时间线视图切换区新增 📥 导出按钮
- `apps/studio/src/views/CharacterChatPage.vue` — `conversationToMarkdown` 增强：用户消息 blockquote + 情感状态摘要
- 10 个单元测试覆盖 CSV 导出边界情况（共 140 个 studio 测试通过）

#### Phase M15：产品稳定性 + AI 对话质量 {#phase-m15}

在推进账号系统之前，先夯实核心体验的稳定性和对话质量，确保现有功能可靠。

- [x] **对话流式输出稳定性** — `streamChat` 新增 `readTimeout`（默认 30s 每 chunk 超时）和 `retries`（默认 2 次自动重试），auth/rate-limit 错误不重试，简单退避策略；`streamToMessage` 默认启用 2 次重试；SSE buffer 尾部残留数据处理
- [x] **useChatStore 上下文裁剪** — 创作助手聊天补齐 token-budget-aware 上下文窗口（6144 token / 30 条硬上限），防止长对话超出 API 限制
- [x] **记忆系统增强** — `userProfile` 上限 30 条（FIFO 淘汰）；`keyEvents` 去重（最近 10 条内子串匹配跳过）；`userInfo` 空值过滤；提取失败 `console.warn` 输出调试信息
- [x] **对话质量评分（数据层）** — `CharacterChatMessage` 新增 `feedback?: 'up' | 'down'` 字段；`setMessageFeedback()` store action + 自动持久化
- [x] **项目验证增强** — 新增角色 `relationships.targetId` 完整性校验（检查目标角色是否存在）
- [x] **单元测试补全** — 新增 `chatUtils.test.ts`（21 tests）+ `projectValidation.test.ts`（8 tests），总计 170 个 studio 单元测试通过
- [x] **多语言对话支持** — AI 系统提示词国际化模板，角色可配置对话语言（中文/英语/日语），知识库检索支持中英日关键词
  - [x] 角色类型新增 `language?: 'zh' | 'en' | 'ja'` 可选字段（`AdvCharacterFrontmatter`）
  - [x] `exportCharacterForAI()` 导出语言信息至 meta 区
  - [x] `buildCharacterSystemPrompt()` 根据角色 `language` 字段动态注入目标语言规则（替代写死的 `ruleUserLanguage`）
  - [x] `characterMd.ts` 解析器支持 `language` frontmatter 字段的序列化/反序列化
  - [x] 角色编辑表单（`CharacterEditorForm`）新增语言下拉选择器（中文/English/日本語/自动）
  - [x] 知识库 `extractKeywords()` 扩展日语分词支持（当前仅覆盖中/英文 stop words，新增日语 stop words + 假名边界分词）
  - [x] 群聊系统提示词（`useGroupChatStore`）同步读取角色 `language` 字段
  - [x] i18n 新增对话语言相关翻译 key（`contentEditor.language` / `contentEditor.languageHint` 等）

**技术实现**：

- `packages/types/src/game/character.ts` — `AdvCharacterFrontmatter` 接口新增 `language?: 'zh' | 'en' | 'ja'`
- `packages/parser/src/character.ts` — `exportCharacterForAI()` meta 区追加 `对话语言: {language}` 行；`serializeCharacterMd()` 支持 language frontmatter
- `apps/studio/src/stores/useCharacterChatStore.ts` — `buildCharacterSystemPrompt()` 第 65-97 行，`PromptBuilder` 链式调用新增 `.addRule(languageRule)` 动态语言规则
- `apps/studio/src/stores/useGroupChatStore.ts` — 群聊角色提示词同步注入目标语言
- `apps/studio/src/composables/useKnowledgeBase.ts` — `extractKeywords()` 第 110 行，新增日语 stop words 列表 + 假名/汉字边界分词
- `apps/studio/src/views/workspace/CharactersPage.vue` — `CharacterEditorForm` 新增语言选择器 UI（中文/English/日本語/自动 四选项）
- `apps/studio/src/i18n/locales/{en,zh-CN}.json` — 新增 `characterEditor.language` 等翻译 key
- [x] **项目健康仪表盘增强** — 现有验证扩展为定期自动检查（进入项目时 + 每次保存后），Dashboard 新增「最近问题」feed，支持一键跳转修复
  - [x] 新增 `ProjectHealthPanel.vue` 组件 — Issue 列表按 `category` 分组（syntax/character/scene/audio/location），严重级别图标 + 问题数 badge
  - [x] Dashboard 集成健康面板 — `ProjectOverview.vue` 替换当前 toast-only 的验证结果展示为持久化 `ProjectHealthPanel` 面板
  - [x] 自动验证触发 — `useProjectContent.loadProjectContent()` 返回后自动调用 `validateProject()`；内容保存（`watchEffect` / `onSave` 回调）后重新触发验证
  - [x] Issue 一键跳转 — 根据 `ValidationIssue.category` + `file` 字段，`router.push` 跳转到对应编辑页（chapters→editor, characters→characters 页, scenes→scenes 页等）
  - [x] i18n 新增健康面板翻译 — `projectHealth.title` / `projectHealth.noIssues` / `projectHealth.issuesByCategory` / `projectHealth.jumpToFix` 等

**技术实现**：

- `apps/studio/src/components/ProjectHealthPanel.vue` — 新增组件，接收 `ValidationResult` prop，按 `issue.category` 分组渲染列表，每项显示文件路径 + 问题描述 + 跳转按钮
- `apps/studio/src/components/ProjectOverview.vue` — 第 113-160 行现有手动验证逻辑保留，但验证结果改为传入 `ProjectHealthPanel` 展示（替代 `presentToast`）
- `apps/studio/src/utils/projectValidation.ts` — 现有 `validateProject()` 返回 `{ issues, passed, stats }` 无需修改，`ValidationIssue` 已有 `category` / `message` / `file` 字段
- `apps/studio/src/composables/useProjectContent.ts` — `loadProjectContent()` 完成后 emit 自动验证；新增 `watch` 对内容变更自动触发 debounced 验证
- `apps/studio/src/components/DashboardSection.vue` — 复用通用分区组件包裹 `ProjectHealthPanel`
- `apps/studio/src/i18n/locales/{en,zh-CN}.json` — 新增 `projectHealth.*` 翻译 key
- [x] **E2E 测试补全** — 覆盖角色对话流、知识库加载、项目导入/导出、群聊创建等核心用户路径（目标 ≥20 个 E2E 用例）
  - [x] `project-management.spec.ts`（4 用例）— 创建新项目 → 验证跳转到 workspace；项目列表切换；项目导出为 zip；zip 文件重新导入并验证内容完整
  - [x] `character-management.spec.ts`（4 用例）— 创建角色 → 验证出现在列表；编辑角色属性（名称/性格/标签）→ 保存 → 验证持久化；批量导入 `.character.md` 文件；删除角色 → 确认对话框 → 验证移除
  - [x] `settings.spec.ts`（3 用例）— AI Provider 配置（切换 provider、填入 API key → 验证保存）；语言切换（中↔英）→ 验证 UI 文本变更；主题切换 → 验证 CSS 变量
  - [x] `world.spec.ts`（3 用例）— 世界页角色列表渲染 → 点击角色跳转到对话页；世界时钟组件交互（日期切换 → 时间线事件过滤）；知识库页文件列表加载
  - [x] `character-chat.spec.ts`（4 用例）— 发送消息 → mock AI 响应 → 验证消息气泡渲染；消息操作（复制/删除/重新生成）→ 验证 UI 状态；存档点创建 → 验证存档列表；👍/👎 反馈 → 验证按钮高亮状态
  - [x] `group-chat.spec.ts`（4 用例）— 创建群聊房间（选择多角色）→ 验证房间列表；群聊发消息 → mock 多角色轮流响应；群聊房间设置修改 → 验证持久化；房间未找到兜底页渲染

**技术实现**：

- `apps/studio/tests/e2e/` — 现有 `test.spec.ts`（4 个基础测试）保留，新增 6 个功能域测试文件
- `apps/studio/playwright.config.ts` — 现有配置（Chromium, baseURL `http://localhost:5173`, testDir `./tests/e2e`）无需修改
- AI 相关测试使用 `page.route('**/v1/chat/completions', ...)` 拦截 API 请求，返回 mock SSE 流式响应
- Ionic 组件选择器使用标签名（`ion-tab-button`, `ion-content`, `ion-modal` 等）
- 测试数据通过 `page.evaluate()` 注入 IndexedDB 初始状态，避免依赖外部文件
- 总计 22 个新增用例 + 4 个现有用例 = 26 个 E2E 用例（超额达成 ≥20 目标）

#### Phase M15b：UI 交互打磨 {#phase-m15b}

基于代码审查发现的 UI 层短板，补齐「可发布品质」的最后一公里。

**角色对话页（CharacterChatPage）**：

- [x] **接入 MessageActions** — 角色对话页集成 `MessageActions.vue` 组件，支持复制/编辑/删除/重新生成操作
- [x] **👍/👎 反馈按钮** — `MessageActions` 新增 feedback prop/emit，点赞点踩按钮调用 `setMessageFeedback()`，选中状态高亮
- [x] **流式内容实时展示** — `streamingContent` 有值时渲染 `MarkdownMessage` 实时气泡，替代空白状态
- [x] **角色未找到兜底页** — `character` 为 `undefined` 时显示 ⚠️ 错误提示 + 返回按钮

**群聊页（GroupChatPage）**：

- [x] **修复空操作** — 移除空函数 `@edit` 绑定，传入 `showEdit=false` 隐藏编辑按钮；`@regenerate` 实现 deleteMessage + generateNextTurn 组合逻辑
- [x] **房间未找到兜底** — `room` 为 `undefined` 时显示错误提示页 + 返回按钮

**项目内容加载（useProjectContent）**：

- [x] **暴露 error 状态** — 新增 `loadErrors: ref<string[]>([])` 收集部分加载失败信息，区分目录不存在（静默）和文件解析失败（收集 + warn）
- [x] **加载失败 toast** — loadErrors 已暴露，消费者可在加载完成后检查并 toast 提示

**时间戳与 i18n**：

- [x] **移动端时间戳可见性** — `chat.css` 添加 `@media (hover: none)` 媒体查询，触屏设备时间戳 opacity 0.6 始终可见
- [x] **i18n 硬编码修复** — `GroupChatPage` 中 `Player`/`Character`/`Group Chat` 替换为 `t('systemPrompt.player')`/`t('world.viewModeCharacter')`/`t('world.groupChats')` i18n key

#### Phase M16：社交分享 + 发布准备 {#phase-m16}

为 Phase 13 账号系统和 Phase 14 市场铺路，建设不依赖后端的社交分享能力。核心目标：让用户能把创作成果「秀出去」。

- [x] **角色卡片分享** — 单个角色可导出为独立 `.character.md` 文件或生成分享图（卡片样式 PNG，含头像/性格/标签/关系）
  - [x] 新增 `modern-screenshot` 依赖（轻量 DOM→Image，替代 html2canvas）
  - [x] 新建 `CharacterShareCard.vue` — 精美分享卡片模板（头像 + 名字 + 阵营 + 性格摘要 + 标签 chips + 关系列表 + ADV.JS 水印），固定 1080×1350 竖版尺寸
  - [x] `CharacterCardActions.vue` 新增「分享」操作按钮 → 弹出 ActionSheet（保存为图片 / 导出 .character.md / 复制角色信息）
  - [x] `shareCharacterAsImage()` — 渲染 `CharacterShareCard` 到隐藏 DOM → `domToPng()` → 下载或 `navigator.share({ files })` 原生分享
  - [x] `shareCharacterAsMd()` — 调用 `stringifyCharacterMd()` 导出 → 下载 `.character.md` 文件
  - [x] i18n 新增 `characters.actionShare` / `characters.shareAsImage` / `characters.shareAsMd` / `characters.shareCopied` 等翻译 key

**技术实现**：

- `apps/studio/package.json` — 新增 `modern-screenshot` 依赖
- `apps/studio/src/components/CharacterShareCard.vue` — 纯展示组件，CSS 固定宽高、不依赖 Ionic 组件（确保截图兼容）
- `apps/studio/src/utils/shareUtils.ts` — `shareCharacterAsImage(character)` / `shareCharacterAsMd(character)` 工具函数
- `apps/studio/src/components/CharacterCardActions.vue` — 新增分享按钮 + ActionSheet
- `apps/studio/src/views/workspace/CharactersPage.vue` — 分享 handler 绑定

- [x] **对话小说导出** — 将完整对话记录导出为排版好的 HTML 页面（可选主题皮肤），支持下载分享
  - [x] 新建 `conversationToHtml()` 函数 — 将对话消息渲染为独立 HTML 页面（内联 CSS，无外部依赖），支持 3 种主题（light/dark/sepia）
  - [x] HTML 模板包含：项目名 + 角色头像/名字 + 对话气泡（用户 vs 角色区分）+ 时间戳 + 情感状态摘要 + ADV.JS 品牌水印
  - [x] `CharacterChatPage.vue` 导出 ActionSheet 新增「导出为 HTML 小说」选项
  - [x] `GroupChatPage.vue` 同步新增 HTML 导出选项（多角色彩色气泡区分）
  - [x] 导出的 HTML 可直接在浏览器打开阅读，文件体积 <100KB

**技术实现**：

- `apps/studio/src/utils/conversationHtml.ts` — `conversationToHtml(messages, character, theme)` 纯函数，返回完整 HTML 字符串
- `apps/studio/src/views/CharacterChatPage.vue` — 导出 ActionSheet 新增 HTML 选项
- `apps/studio/src/views/GroupChatPage.vue` — 同上
- `apps/studio/src/i18n/locales/{en,zh-CN}.json` — 新增 `world.exportHtml` / `world.exportThemeLight` / `world.exportThemeDark` / `world.exportThemeSepia`

- [ ] **项目封面与简介** — 项目可上传/AI 生成封面图，配置项目简介文本，用于分享预览和未来市场展示
  - [ ] `ProjectSettings` 新增 `cover?: string`（base64/URL）和 `description?: string` 字段
  - [ ] `ProjectSettingsModal.vue` 新增封面上传区域（点击上传 + AI 生成 fallback）+ 简介文本框
  - [ ] 封面图存储在 `adv/cover.png`（随项目导出/导入自动包含）
  - [ ] `ProjectOverview.vue` header 展示封面缩略图（如果有）
  - [ ] `exportProject()` manifest 新增 `cover` 和 `description` 字段

**技术实现**：

- `apps/studio/src/stores/useStudioStore.ts` — `ProjectEntry` 新增 `cover?` / `description?`
- `apps/studio/src/components/ProjectSettingsModal.vue` — 封面上传 + 简介编辑
- `apps/studio/src/composables/useProjectExport.ts` — manifest 扩展
- `apps/studio/src/components/ProjectOverview.vue` — 封面缩略图展示
- `apps/studio/src/i18n/locales/{en,zh-CN}.json` — 新增 `projectSettings.cover` / `projectSettings.coverUpload` / `projectSettings.coverAiGenerate` / `projectSettings.description` 等

- [ ] **分享链增强** — 项目分享时附加封面图 + 简介，通过 Web Share API 分享图片文件；静态预渲染 OG meta 支持（微信/Twitter 预览卡片）
  - [ ] `handleShare()` 重构：如果项目有 cover，构造 `navigator.share({ files: [coverFile], title, text })` 带图分享
  - [ ] 无 cover 时 fallback 到现有的纯文本 + URL 分享
  - [ ] `index.html` 新增默认 OG meta 标签（`og:title` = ADV.JS Studio / `og:description` / `og:image` = 默认品牌图）
  - [ ] vite 插件 `vite-plugin-html-prerender`（可选）或手动在 `index.html` 注入静态 OG meta

**技术实现**：

- `apps/studio/src/components/ProjectOverview.vue` — `handleShare()` 增强
- `apps/studio/src/views/PlayPage.vue` — `handleShare()` 同步增强
- `apps/studio/index.html` — 静态 OG meta 标签
- `apps/studio/src/i18n/locales/{en,zh-CN}.json` — 新增 `dashboard.shareWithCover` 等

#### Phase M17：性能 + 原生准备 {#phase-m17}

优化运行时性能，为 Capacitor 原生打包扫清障碍。

- [x] **虚拟滚动实装（对话消息）** — `CharacterChatPage` + `GroupChatPage` 已接入 `VirtualMessageList`（@tanstack/vue-virtual），支持 1000+ 条消息无感滚动
- [ ] **IndexedDB 查询优化** — 为高频查询路径（角色消息加载、知识库检索、日记列表）添加复合索引，批量读写使用 `db.transaction`
- [ ] **Bundle 体积优化** — Monaco Editor 按需加载语言包（目前加载全部语言），Ionic 组件 tree-shaking 审计，目标减少 30% 初始包体积
- [x] **文件系统兼容层（IFileSystem 抽象）** — 定义统一 `IFileSystem` 接口，实现三个适配器：`BrowserFsAdapter`（File System Access API）、`CapacitorFsAdapter`（`@capacitor/filesystem` 插件）、`MemoryFsAdapter`（IndexedDB 持久化内存 FS）。工厂函数 `createFileSystem()` 自动选择后端
- [x] **消费者迁移** — 旧 `fileAccess.ts` 中的工具函数迁移至 `fs/utils.ts`，10 个消费者文件导入路径全部更新，三个适配器去重统一引用 `fs/utils.ts`，`ProjectsPage.vue` 的 `autoLoadReadme` 改用 `IFileSystem.readFile`，`fileAccess.ts` 已删除
- [ ] **Capacitor iOS/Android 工程** — 初始化 Capacitor 项目配置，配置图标/启动屏，完成首次本地构建和模拟器运行

::: tip 进展说明
Phase M1-M14 + Phase L1-L3 全部完成（60+ 个组件、32+ 个页面、13 个 Store、170+ 个单元测试）。
...
M17 文件系统兼容层已完成（IFileSystem 接口 + 3 个适配器 + 工厂函数 + DB v12），消费者迁移已完成（`fileAccess.ts` 已删除，所有消费者统一使用 `fs/` 模块），对话虚拟滚动已实装。
...
对话是 Studio 最高频的交互。当前痛点：首次加载慢、流式输出偶发中断后无恢复提示。

- [x] **虚拟滚动实装（对话消息）** — `CharacterChatPage` + `GroupChatPage` 已接入 `VirtualMessageList`（@tanstack/vue-virtual），消息列表视口渲染，1000+ 条消息无感滚动
...
M17 已建立 `IFileSystem` 抽象层。消费者迁移已完成：`fileAccess.ts` 已删除，所有工具函数统一到 `fs/utils.ts`，核心 composable 全部通过 `IFileSystem` 接口访问文件。剩余工作为 Capacitor 原生构建。

- [x] **消费者迁移（批次 1）** — `useProjectContent` + `useContentSave` + `useContentDelete` 已使用 `IFileSystem`，`fileAccess.ts` 工具函数迁移至 `fs/utils.ts`
- [x] **消费者迁移（批次 2）** — `useKnowledgeBase` + `useProjectExport` + `useCloudSync` 已通过 `createFsForProject` 使用 `IFileSystem`
- [x] **消费者迁移（批次 3）** — 10 个组件/页面导入路径全部从 `fileAccess` 更新到 `fs`，`ProjectsPage.vue` 的 `autoLoadReadme` 改用 `IFileSystem.readFile`
...
- [x] **虚拟滚动实装** — 对话页面已接入 VirtualMessageList（@tanstack/vue-virtual），角色列表/时间线后续跟进（→ Phase N4）
- [ ] **一键体验模式** — 零配置快速开始对话体验（→ Phase N2）

### 地点系统路线 {#location-roadmap}

地点（Location）模块为 Studio 新增的世界观组织维度，分阶段实现：

#### Phase L1：基础 CRUD ✅ {#phase-l1}

- [x] 地点 Markdown 解析/序列化（`locationMd.ts`）
- [x] 地点卡片组件 + 编辑表单（`LocationCard.vue` + `LocationEditorForm.vue`）
- [x] 地点列表页（`LocationsPage.vue`）— 搜索、创建、编辑、删除、草稿恢复
- [x] 通用 composable 扩展（`useProjectContent` / `useContentEditor` / `useContentSave`）
- [x] 路由注册 + i18n（中/英）
- [x] Dashboard 统计卡 + 地点地图入口卡
