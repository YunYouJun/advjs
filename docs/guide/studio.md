...
- [ ] **多语言对话支持** — AI 系统提示词国际化模板，角色可配置对话语言（日语/英语/韩语等），知识库检索支持多语言关键词
- [ ] **项目健康仪表盘增强** — 现有验证扩展为定期自动检查（进入项目时 + 每次保存后），Dashboard 新增「最近问题」feed，支持一键跳转修复
- [ ] **E2E 测试补全** — 覆盖角色对话流、知识库加载、项目导入/导出、群聊创建等核心用户路径（目标 ≥20 个 E2E 用例）
...
#### Phase M16：社交基础 + 发布准备 {#phase-m16}

为 Phase 13 账号系统和 Phase 14 市场铺路，建设不依赖后端的社交分享能力。

- [ ] **项目分享链增强** — 生成可预览的分享页面（项目封面 + 简介 + 角色数/章节数统计），支持 Open Graph 元数据（微信/Twitter 预览卡片）
- [ ] **角色卡片分享** — 单个角色可导出为独立 `.character.md` 文件或生成分享图（卡片样式 PNG，含头像/性格/标签/关系）
- [ ] **对话小说导出** — 将完整对话记录导出为排版好的 HTML 页面（可选主题皮肤），支持分享阅读链接
- [ ] **项目封面生成器** — 基于项目角色和世界观，AI 生成或手动上传项目封面图（用于市场展示）
...
M15 稳定性核心项 + M15b UI 打磨已完成。

**下一阶段路线**：M15（剩余项收尾）→ M16（社交分享）→ M17（性能/原生）→ Phase 13（账号系统）→ Phase 14（世界市场）
