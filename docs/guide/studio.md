...
- [ ] **Capacitor 兼容层** — 抽象文件系统 API 为统一接口（浏览器 File System Access API / Capacitor Filesystem 插件 / 内存 fallback），确保同一套代码在 Web/iOS/Android 上运行
...
**下一阶段路线**：M16 剩余（项目封面 + 分享链增强，可选）→ M17（性能/原生，关键：Capacitor 文件系统兼容层）→ Phase 13（账号系统）→ Phase 14（世界市场）
